...
(function ($) {
  "use strict"; // This set of validators requires the File API, so if we'ere in a browser
  ...
  $.validator.addMethod("fileType", function (value, element, params) {
    ...
    return formatter("File must be one of the following types: {0}.", params.types.join(","));
  });
  /**
  ...
  $.validator.addMethod("minFileSize", function (value, element, params) {
    var files,
        unit = params.unit || "KB",
    ...
    return formatter("File must be at least {0}{1} large.", [params.size || 100, params.unit || "KB"]);
  });
  /**
  ...
  $.validator.addMethod("maxFileSize", function (value, element, params) {
    ...
    return formatter("File cannot be larger than {0}{1}.", [params.size || 100, params.unit || "KB"]);
  });
})(jQuery);